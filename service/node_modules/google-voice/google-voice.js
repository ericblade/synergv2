...
function getXMLPage(gv,options,callback){
	...
	gvrequest(gv,requestOptions,function(status,httpResponse,body, error){
		...
		if(status){
			...
			fs.writeFileSync('./parserinput.txt', body);
			var exec = child_process.exec("node doParsing.js", function(error, stdout, stderr) {
				if(error) {
			...
			exec.on('exit', function(code) {
				...
				var parserdata = fs.readFileSync('./parseroutput.txt');
...
function processMessages(messages, html, callback){
	...
	if(anySms) {
		fs.writeFileSync('./jsdominput.txt', html);
		fs.writeFileSync('./jsdommessages.txt', JSON.stringify(messages));
		console.log("starting child process to process dom");
		var exec = child_process.exec("node doJSDOM.js", { timeout: 10000 }, function(error, stdout, stderr) {
		...
		exec.on('exit', function(code) {
			console.log("child process exited with code " + code);
			var jsdomdata = fs.readFileSync('./jsdomoutput.txt');
