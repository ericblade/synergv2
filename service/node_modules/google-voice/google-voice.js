...
function getXMLPage(gv,options,callback){
	...
	gvrequest(gv,requestOptions,function(status,httpResponse,body, error){
		...
		if(status){
			...
			fs.writeFileSync('./parserinput' + randId + '.txt', body);
			...
			exec.on('exit', function(code) {
				...
				var parserdata = fs.readFileSync('./parseroutput' + randId + '.txt');
...
function processMessages(messages, html, callback){
	...
	if(anySms) {
		fs.writeFileSync('./jsdominput' + randId + '.txt', html);
		fs.writeFileSync('./jsdommessages' + randId + '.txt', JSON.stringify(messages));
		...
		exec.on('exit', function(code) {
			console.log("child process exited with code " + code);
			var jsdomdata = fs.readFileSync('./jsdomoutput' + randId + '.txt');
